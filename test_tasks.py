...
def test_no_page_0_people_req():
    response0 = requests.get(base_url + '/people/?page=0')
    response1_prev = requests.get(base_url + '/people/?page=1')
    assert ((response0.status_code == 404) and (response1_prev.json()['previous'] is None))
...
def test_names_count(count, name):
    response = requests.get(base_url + '/people/?search=' + name)
    assert response.status_code == 200
    assert ((int(response.json()['count']) == count) and (len(response.json()['results']) == count))
...
def test_people_obj_contain_required_schema(api_get_schema_people_obj, api_get_people):
    required_fields = set(api_get_schema_people_obj['required'])
    for creature in api_get_people:
        creature_fields = set(creature.keys())
        assert required_fields.issubset(creature_fields)


def test_people_obj_contain_required_schema_ok(api_get_schema_people_obj_ok, api_get_people):
    required_fields = set(api_get_schema_people_obj_ok['required'])
    for creature in api_get_people:
        creature_fields = set(creature.keys())
        assert required_fields.issubset(creature_fields)
...
def test_search_any_char_not_empty_result(api_get_search_result, test_arg):
    ...
    if test_arg not in '069':
        assert ((res['count'] > 0) and (len(res['results']) > 0))
    else:
        assert ((res['count'] == 0) and (len(res['results']) == 0))
